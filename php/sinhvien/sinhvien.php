            ...
            <div class="main-r col-md-9">               
                ...
                <form class="frm" action="" method="post">
                    <div class="row row-qlmh">    
                        ...
                        <div class="value"><input type="text" value = "'.$_SESSION['username'].'" name="svma" size="15" maxlength="8" required></div>
                    </div>';
                        ?>
                        <div class="col-md-12">
                ...
                <table class="table-data" bgcolor="#FFFFFF">
                    ...
                    {   
                        ...
                        if(isset($_POST['svnh'])){
                            ...
                            $output = ''; //khai bao chuoi de echo xuat du lieu
                            $query = "SELECT diem.MaSV,giaidoan.MaGD,monh.TenMH,diem.DQT,diem.DT,diem.diemtongket  FROM giaidoan,lophp,monh,diem,dsl WHERE giaidoan.MaGD = lophp.MaGD AND lophp.MaLHP = dsl.MaLHP AND lophp.MaMH = monh.MaMH AND diem.MaMH=monh.MaMH AND diem.MaSV = (SELECT sv.MaSV FROM sv WHERE sv.MaSV = '$masv' AND dsl.MaSV ='$masv')";
                            $kq = mysqli_query($dbcon,$query); //truyen sql vao mysql
                            while($row = mysqli_fetch_array($kq) ) //ham tra ve tat ca ket qua
                            {
                                $output .= '<tr><td>' .$row[0]. '</td><td>' .$row[1]. '</td><td>'.$row[2].'</td><td>'.$row[3].'</td><td>'.$row[4].'</td><td>'.$row[5].'</td></tr>';
